...
resource "google_compute_instance" "igv-webapp" {
  boot_disk {
    auto_delete = true
    device_name = "igv-webapp"
  ...
  provisioner "remote-exec" {
    ...
    inline = [
	  "docker run -dt -p 8080:8080 --name igvwebapp ghcr.io/lescai-teaching/igv-webapp:latest"
    ]
  }
